...
<%
    cp = sqlCheck(replace(request("cp"),"'","''"))
    msgtype = sqlCheck(replace(request("msgtype"),"'","''"))


    sql = "exec PRC_tb_SMSPush_SMS_List N'" & cp & "', '" & msgtype & "', '', ''"
    ...
    if msgtype = "s" then ''sms
        sql = "Select nvcCall From tb_CP Where nvcCPCode = N'" & cp & "'"
...
<%
    ...
    returnVal = returnVal & "<t><c>" & cpTel & "</c></t>"
