...
function fn_goRegistClass()
{
    ...
    line-height: 1.5;
      ...
      table.calendar td {
		...
		.month_title{font-size: 24px; margin: 0 0 10px 50px; font-weight: bold;float: left;}
...
<div class="contents">
  ...
  <div class="usetalking_ing">
    <table class="type07" style="width:100%;">
	  ...
	  <div class="text_list">
		 <ul><br>
		 <li class="text_style5">｜강사를 클릭한 후, 플레이타임/시간대를 변경하셔서 검색하세요.</li><br>
		 <li></li><br>
...
<%
	...
	<div style="margin-left: 10px;width:800px;float:left;margin-bottom:30px;">
		...
		<div style="margin-top: 96px;width:200px;float:left;margin-right:10px;height:100%;">
		...
		Else
				...
				if iTeacherSeq <> "" Then
					...
					If isarray(arrTeacher) Then
						nvcTeacherWorkTime = arrTeacher(5,0)
						nvcTeacherName  = arrTeacher(12,0)
					End If
				...
				If iTeacherSeq <>"" then
					...
					'강사의 휴강일 구하기
				  sql = "exec PRC_tb_Teacher_Select_BDate_View '"&iTeacherSeq &"'"
					set rs = dbSelect(sql)
					...
					If isArray(arrBDate) Then
						maxLenTHoli = ubound(arrBDate, 2)
						for iForr = 0 to maxLenTHoli
							tot_TeacherBreak = tot_TeacherBreak & " " &  arrBDate(2, iForr)				
						Next 
					End If
					...
					If isArray(arrTeacher) then
						If arrTeacher(5,0) <> "" Then	
							...
							For idx=0 To ubound(arrCTime)	'시간이 14시부터 23시까지
								If arrCTime(idx) > 6 And arrCTime(idx) < 24 Then

									m_hour = CInt(arrCTime(idx)&"")
		...
		<table class="calendar">
		 <thead>
            <tr >
                <th scope="cols" colspan="3" style="background-color:#ffffff;">플레이타임  : <%=nowRunTime%> min
            ...
            <tbody>
			 ...
			 If searchStartDate <= Trim(tmpDate&"") And searchEndDate >= Trim(tmpDate&"") And nowRunTime <> "" And m_Hour <> "" And  InStr(tot_TeacherBreak,Trim(tmpDate&"")) <= 0 Then
							%>
						...
						If holidaycheck = 0 then
							...
							If  to_week <> 1 And to_week <> 7 And  instr(BalTime,tmpDate) > 0 And Trim(tmpdate&"") <= Trim(searchlimitdate&"") Then 
										...
										If BalClassStime0 <> "" Then 
											 ...
											 If chk(arrBreak,BalClassStime0,to_week) And chk2(arr_ClTime,tmpDate,BalClassStime0)   And trim(tmpDate&"") >= searchStartDate Then 
													Rowspannum0 = 1
											End If 
										...
										If BalClassStime1 <> "" Then 
											 ...
											 If chk(arrBreak,BalClassStime1,to_week) And chk2(arr_ClTime,tmpDate,BalClassStime1)   And trim(tmpDate&"") >= searchStartDate Then 
													Rowspannum1 = 1
											End If 
										...
										If BalClassStime2 <> "" Then 
											 ...
											 If chk(arrBreak,BalClassStime2,to_week) And chk2(arr_ClTime,tmpDate,BalClassStime2)   And trim(tmpDate&"") >= searchStartDate Then 
													Rowspannum2 = 1
											End If 
										...
										If BalClassStime3 <> "" Then 
											 ...
											 If chk(arrBreak,BalClassStime3,to_week) And chk2(arr_ClTime,tmpDate,BalClassStime3)   And trim(tmpDate&"") >= searchStartDate Then 
													Rowspannum3 = 1
											End If 
										...
										If BalClassStime4 <> "" Then 
											 ...
											 If chk(arrBreak,BalClassStime4,to_week) And chk2(arr_ClTime,tmpDate,BalClassStime4)   And trim(tmpDate&"") >= searchStartDate Then 
													Rowspannum4 = 1
											End If 
										...
										If BalClassStime5 <> "" Then 
											 ...
											 If chk(arrBreak,BalClassStime5,to_week) And chk2(arr_ClTime,tmpDate,BalClassStime5)   And trim(tmpDate&"") >= searchStartDate Then 
													Rowspannum5 = 1
											End If 
									...
									%>
									<tr >
									<td height="20"  rowspan="<%=RowspannumAll-1%>" colspan="2"><span style="padding-left:6px;"><%=Month(tmpDate)&"월 "&printDate&"일 (" & fn_strweek(weekday(trim(tmpDate&""))) & ") "%></span>
									</td>		
				...
				<form name="frmselclass" id ="frmselclass" method="post" action="/MyPage/MypageClassOrder_ok.asp">			
						...
						<div class="usetalking_ing">
							<table class="type07" style="width:100%;" border=1>
							   ...
							   <tr>
								   <th style="border-top: 1px solid #ccc;width:200px;">선택한 강사 </th>
								   <td style="border-top: 1px solid #ccc;" colspan="2"><%=nvcTeacherName%></td>			  
							   </tr>
