...
iDailySeq= sqlCheck(Replace(Request("iDailySeq"), "'", "''"))
...
searchlimitdate = Left(dateadd("d",-7,searchEndDate),10)
...
sql = sql & " ts.nvcScheEndDate,ts.iCourseSeq "
...
sql = sql & "  where tdr.siAttendance in (0) and (tsd.sischeflag in (1,2) or "
sql = sql & " (tsd.sischeflag = 3 and isnull(tsd.nvcScheExpire,'') <= '" & Left(now(),10) & "')) "
sql = sql & " and tdr.iDailyReportSeq ='" & iDailySeq & "' and tdr.nvcDailyReportDate > '" & Left(now(),10) & "'  "
...
End If 
'Response.write " iDailyReportSeq " & iDailyReportSeq
'Response.write " iScheduleSeq " & iScheduleSeq
'Response.write " nowRunTime " & nowRunTime
'Response.write " iTeacherSeq " & iTeacherSeq
...
<!--#include virtual="/include/inc_navi.asp"-->
  ...
  <div class="usetalking_ing">
    <table class="type07" style="width:100%;">
	  ...
	  <div class="text_list">
		...
		<ul>	
		 <li> 1) 오른쪽 상단 [시간] 선택 (기존 강사의 "빈 수업시간"이 달력에 표시됩니다.)</li>
		 <li> 2) 달력에서 원하는 날짜의 시간(빨간색)을 클릭 </li>
		 <li> 3) 알림창 "확인" 클릭 >> 1회 수업이 변경됩니다.</li>
		 </ul>
...
<%
		...
		strSQL = " EXEC PRC_Front_tb_Teacher_Select_All '" & iCourseSeq & "' "
